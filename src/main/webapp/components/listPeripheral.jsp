<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>


<div class="container my-4">
	<div id="productPeripheral" class="row">
		<c:if test="${not empty ListPeripheral}">
			<c:forEach var="peripheral" items="${ListPeripheral}">
				<div class="peripheralItem col-6 col-md-3 mb-4">
					<div class="card">
						<div class="card-body" style="margin-top: -13px">
							<a href="http://" style="text-decoration: none; color: black">
								<img
								src="${peripheral.image}"
								class="card-img-top" alt="..." />
								<h6 class="mb-2 card-title" style="padding-left: 30px">
									${peripheral.name}</h6>

<%-- 								<div class="mb-2 bg-info-subtle rounded-1 d-flex flex-wrap"
									style="font-size: 15px; padding-left: 24px">
									<div class="me-3 ms-1">
										<i class="bi bi-cpu-fill me-1"></i>${laptop.cpu}
									</div>
									<div class="me-3 ms-1">
										<i class="bi bi-gpu-card me-1"></i>${laptop.gpu }
									</div>
									<div class="me-3 ms-1">
										<i class="bi bi-memory me-1"></i>${laptop.ram }
									</div>
									<div class="me-3 ms-1">
										<i class="bi bi-device-ssd-fill me-1"></i>${laptop.drive }
									</div>
									<div class="me-3 ms-1">
										<i class="bi bi-tv me-1"></i>${laptop.size} ${laptop.resolution}
									</div>
								</div> --%>

								<div class="text-danger text-center" style="font-size: 20px">
									<span id="price"> <c:out value="${String.format('%,d', peripheral.price)} đ" /></span>
								</div>

								<div style="font-size: 15px"
									class="mb-1 d-flex justify-content-center">
									<span class="text-warning me-1">0.0</span> <i
										class="bi bi-star-fill text-warning me-1"></i> (0 đánh giá)
								</div>
							</a>
							<div class="col d-flex justify-content-between">
								<a href="#" class="btn btn-outline-primary"
									style="font-size: 13px"> Mua Ngay </a> <a href="#"
									title="thêm vào giỏ hàng" class="btn btn-outline-primary"
									style="font-size: 13px"> <i class="bi bi-cart-plus-fill"></i>
								</a>
							</div>
							
						</div>
					</div>
				</div>
			</c:forEach>
		</c:if>
	</div>
	<div class="d-flex justify-content-center"><button class="btn btn-primary" onclick="moreProduct()">Xem thêm</button></div>
	<script
      src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.1/jquery.min.js"
      integrity="sha512-v2CJ7UaYy4JwqLDIrZUI/4hqeoQieOmAZNXBeQyjo21dadnwR+8ZaIJVT8EE2iyI61OV8e6M8PP2/4hpQINQ/g=="
      crossorigin="anonymous"
      referrerpolicy="no-referrer"
    ></script>
     <script>
     var amount = document.getElementsByClassName("peripheralItem").length;
     function moreProduct() {
         $.ajax({
           url: "http://localhost:8080/LapTopStore/loadMoreProducts",
           type: "get",
           dataType: "html",
           data: {
             productExit: amount,
           },
           success: function (respone) {
             var productMore = document.getElementById("productPeripheral");
             productMore.innerHTML +=respone;
             amount += 4;
           },
         });
       }
    </script>
	
</div>