...
public class OrderDetailDAO {
	...
	public List<OrderDetail> getListOrderDetail(int orderId) {
		...
		try {
			...
			String query = "select o.orderId, o.productId, p.name,p.image,p.price, o.quantity,p.type from orderDetail o join product p on o.productId=p.productId where o.orderId=?";
			...
			while (rs.next()) {
				OrderDetail orderDetail = new OrderDetail(rs.getInt("orderId"), rs.getInt("productId"),
						rs.getString("name"), rs.getString("image"), rs.getInt("price"), rs.getInt("quantity"),rs.getString("type"));
